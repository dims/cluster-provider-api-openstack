...
func (s *Service) CreateInstance(openStackCluster *infrav1.OpenStackCluster, machine *clusterv1.Machine, openStackMachine *infrav1.OpenStackMachine, clusterName string, userData string) (instance *infrav1.Instance, err error) {
	...
	// Get security groups
	securityGroups, err := s.constructSecurityGroups(openStackCluster, machine, openStackMachine.Spec.SecurityGroups)
	if err != nil {
		return nil, err
	}
...
func (s *Service) constructSecurityGroups(openStackCluster *infrav1.OpenStackCluster, machine *clusterv1.Machine, specSecurityGroups []infrav1.SecurityGroupParam) (*[]string, error) {
	securityGroups, err := s.networkingService.GetSecurityGroups(specSecurityGroups)
	if err != nil {
		return nil, err
	}
...
func (s *Service) createInstance(eventObject runtime.Object, clusterName string, instance *infrav1.Instance) (*infrav1.Instance, error) {
	...
	for i, network := range *instance.Networks {
		...
		if instance.Trunk {
			_, err := s.networkingService.GetOrCreateTrunk(eventObject, clusterName, instance.Name, port.ID, instance.Tags)
			if err != nil {
				return nil, err
			}
		}
